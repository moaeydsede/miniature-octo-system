...
function defaultState(){
  ...
  return {
    ...
    departments: [
      { id: crypto.randomUUID(), name: "إدارة" },
      { id: crypto.randomUUID(), name: "مبيعات" },
      { id: crypto.randomUUID(), name: "مخزن" },
      { id: crypto.randomUUID(), name: "إنتاج" }
    ],
    employees: [
      { id: crypto.randomUUID(), name: "محمد", code: "E001", departmentId: null, monthlySalary: 6000, active: true },
      { id: crypto.randomUUID(), name: "أحمد", code: "E002", departmentId: null, monthlySalary: 5500, active: true }
    ],
    settingsByMonth: {
      [ymKey(y,m)]: {
        year:y, month:m,
...
function addAudit(st, action, payload){
  st.audit.unshift({ id: crypto.randomUUID(), at: new Date().toISOString(), action, payload });
  st.audit = st.audit.slice(0, 500);
}

function requireRole(role){
  const s = sessionStorage.getItem("HR_SESSION");
...
function setSession(sess){
  sessionStorage.setItem("HR_SESSION", JSON.stringify(sess));
}
